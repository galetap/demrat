...
#' \code{Age_max}, upper limit of age-at-death interval, must be a numeric variable.
...
age_div <- function(data, show_res = FALSE, extra_var=NULL) {
  if(length(dplyr::intersect(c("Age_min", "Age_max"), colnames(data)))!=2){
    ...
    data <-
      ...
      {if("Culture" %in% colnames(.))
        .
          ...
          mutate(., Culture="Unknown")} %>%

      # Subset of complete cases (cases with Age_min and Age_max or Culture)
      select(Site, Culture, {{extra_var}}, Age_min, Age_max) %>%
      drop_na() %>%
      # Convert Age_min and Age_max to numeric format
      mutate(across(.cols = c(Age_min, Age_max),
                    .fns = as.numeric))
    ...
    res <-
      distinct %>%
      cbind(dist_div) %>%
      left_join(data, ., by = c("Age_min", "Age_max")) %>%
      as.data.frame()
